...
Resolution sources, in priority order (**revised by M0 findings**, 2026-07-13 — local extraction was investigated and found not viable: current-client files store no structural ID→name data; the char file doesn't even contain its own character ID, and all in-file name occurrences are incidental UI state. See format-notes.md *Name presence*):

1. **ESI (primary, character IDs only).** Character IDs resolve via ESI's public `POST /universe/names` endpoint — no authentication, batched (one request for all unresolved IDs), cached persistently in the app's data directory. Offline or on ESI failure, the UI falls back to bare IDs; cached names keep working. A settings toggle disables all network access; this is the app's **only** network call. Account IDs have no public API and never resolve online.
2. **User aliases (accounts).** The user can assign a persistent **alias** to any account ID.
3. **Correlation suggestions (accounts, confirmed viable in M0).** Char and user files are written seconds apart at logout, so mtime clustering suggests "account of character *X*"; a weak in-file hint (capsule container-window ids in the user file embed the account's characters' names) can corroborate. Suggestions are clearly marked as guesses and confirmable into an alias with one click; they are never displayed as authoritative.
...
| Names next to IDs | ESI primary for character IDs; aliases + mtime-correlation suggestions for accounts (M0 finding: no structural names/IDs inside the files — local extraction rejected) | M0 investigation: fresh files hold names only incidentally (channel labels, capsule window ids), no ID→name link anywhere. Accounts have no public API. |
